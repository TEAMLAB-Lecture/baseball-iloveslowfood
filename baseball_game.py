...
def is_between_100_and_999(user_input_number: str) -> bool:
    ...
    return True if number >= 100 and number < 1000 else False
...
def get_not_duplicated_three_digit_number():
    ...
    while not is_validated_number(candidate_num):
        candidate_num = str(random.randint(100, 999))
    return int(candidate_num)
...
def get_strikes_or_ball(user_input_number: str, random_number: str) -> list:
    ...
    Examples:
      ...
      [3, 0]
    """
    n_strikes = sum([i == j for i, j in zip(user_input_number, random_number)])
    n_balls = len(set(user_input_number).intersection(random_number)) - n_strikes

    return [n_strikes, n_balls]
